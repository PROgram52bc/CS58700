...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x, y = torch.tensor(self.tokens[idx:idx+self.block_size]), torch.tensor(self.tokens[idx+1:idx+1+self.block_size])
        ### END OF TODO ###
        return x, y
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, seq_len):
        ...
        # Shape: (seq_len, dim/2)
        angles = position[:, None] * self.inv_freq[None, :] # shortcut for unsqueeze
...
def apply_rotary_emb(q, k, sin, cos, pos=None):
    """
    ...
    q1, q2 = q[..., ::2], q[..., 1::2] # (batch_size, num_heads, seq_len, head_dim / 2)
    k1, k2 = k[..., ::2], k[..., 1::2] # (batch_size, num_heads, seq_len, head_dim / 2)
    ...
    if q.size(2) != 1:
        ...
        # q's size is only 1: apply rotary using only the current position
        assert pos is not None
        new_sin = sin[:,:,pos] # (1,1,1,head_dim/2)
        new_cos = cos[:,:,pos] # (1,1,1,head_dim/2)
        q_rotated = torch.cat([q1 * new_cos - q2 * new_sin, q1 * new_sin + q2 * new_cos], dim=-1)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None, use_cache=False):
        ...
        # (B, T, 3 * C)
        q, k, v = self.c_attn(x).split(x.shape[2], dim=2)
        # (B, T, C)
        ...
        v = v.view(B, T, nh, head_dim).transpose(1,2)
        ...
        pos = past_k.size(2) if layer_past is not None else None
        q, k = apply_rotary_emb(q,k,sin,cos,pos=pos)

        # Compute attention scores
        # (B, nh, T, hs) x (B, nh, hs, seq_len) -> (B, nh, T, seq_len)
        # T = seq_len
        attn = torch.matmul(q, k.transpose(-1,-2)) / math.sqrt(head_dim)

        # assert attn.size(2) == attn.size(3)
        ...
        if layer_past is None:
            ...
            # pay no attention to future tokens
            mask = self.mask[:,:,:attn.shape[-2],:attn.shape[-1]]
...
if __name__ == "__main__":
        gpt = MyGPT()
        print(gpt.generate("Why is the sky blue?", max_tokens=50))
