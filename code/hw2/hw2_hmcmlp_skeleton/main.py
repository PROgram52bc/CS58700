...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('app.log'), logging.StreamHandler()]  # Log to file and console
)
...
def main():
    ...
    if utils.loaded == False:
        ...
        utils.plot_graph([i + 1 for i in range(utils.epochs)], train_accs, "Epochs", "Accuracy",
                         "Train Epoch vs Accuracy", './plots/' + utils.depth + "_mle_train_accuracy.jpg")
        utils.plot_graph([i + 1 for i in range(utils.epochs)], test_accs, "Epochs", "Accuracy",
                         "Test Epoch vs Accuracy", './plots/' + utils.depth + "_mle_test_accuracy.jpg")
        utils.plot_graph([i + 1 for i in range(utils.epochs)], losses, "Epochs", "Loss", "Train Epoch vs Loss",
    ...
    bayesian_y_train_pred = torch.max(bayessian_y_train_prob, 1)[1].to(
        utils.device)  # new_bayesian_model.predict(X_train)
    ...
    """
    Step 4: Draw Reliablity Diagrams
    """

    print("ECE for MLE in training:")
    utils.analyse_calibration(y_train, y_train_prob, utils.num_bins,
                              './plots/' + utils.depth + "_mle_train_reliablity.jpg", threshold=0.5)
    print("ECE for HMC in training:")
    utils.analyse_calibration(y_train, bayessian_y_train_prob, utils.num_bins,
                              './plots/' + utils.depth + "_bayessian_train_reliablity.jpg", threshold=0.5)
    print("ECE for pertubated HMC in training:")
    utils.analyse_calibration(y_train, perturbed_bayessian_y_train_prob, utils.num_bins,
                              './plots/' + utils.depth + "_perturbed_bayessian_train_reliablity.jpg", threshold=0.5)

    print("ECE for MLE in test:")
    utils.analyse_calibration(y_test, y_test_prob, utils.num_bins,
                              './plots/' + utils.depth + "_mle_test_reliablity.jpg", threshold=0.5)
    print("ECE for HMC in test:")
    utils.analyse_calibration(y_test, bayessian_y_test_prob, utils.num_bins,
                              './plots/' + utils.depth + "_bayessian_test_reliablity.jpg", threshold=0.5)
    print("ECE for pertubated HMC in test:")
    utils.analyse_calibration(y_test, perturbed_bayessian_y_test_prob, utils.num_bins,
                              './plots/' + utils.depth + "_perturbed_bayessian_test_reliablity.jpg", threshold=0.5)
