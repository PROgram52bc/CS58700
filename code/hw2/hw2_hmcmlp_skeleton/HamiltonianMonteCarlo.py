...
class HamiltonianMonteCarloSampler:
    ...
    def get_sampled_velocities(self, stddv):
        ...
        """
        return [ torch.randn_like(param) * stddv for param in self.model.parameters() ]
    ...
    def leapfrog(self, velocities, delta, /, *ARGS):
        ...
        *ARGS : (X, y, y_1hot) as described in utils.py and NeuralNetwork model learning
        ...
        U = self.model.energy(*ARGS)
        U.backward()

        for parameter, velocity in zip(self.model.parameters(), velocities):
        # 1. phi half = phi sent - delta half * U grad step
            velocity_half = velocity - 0.5 * delta * parameter.grad

        # 2. X new = X old + delta * phi half step, X is the model parameters
            with torch.no_grad():
                parameter += delta * velocity_half
        # 3. phi new = phi half - delta half * U grad new step
            # recompute gradient since parameter is updated
            self.model.zero_grad()
            U = self.model.energy(*ARGS)
            U.backward()
            new_velocity = velocity_half - 0.5 * delta * parameter.grad
    ...
    def accept_or_reject(self, potential_energy_previous, potential_energy_current, 
        ...
        """
        curr = potential_energy_current + kinetic_energy_current
        prev = potential_energy_previous + kinetic_energy_previous
        acc = min(1, torch.exp(torch.tensor(curr - prev)))
        if torch.rand(1) < acc:
            return True
        else:
            return False
    ...
    def sample(self, n, std_dev, delta, num_leapfrogs, /, *ARGS):
        ...
        *ARGS : (X, y, y_1hot) as described in utils.py and NeuralNetwork model learning
        ...
        for (param, init) in zip(self.model.parameters(), inits):
            # manually update the parameters without triggering autograd
            param.data.copy_(init)

        with torch.no_grad():
            # compute the loss/potential energy
