...
class GCN(torch.nn.Module):
    ...
    def forward(
        ...
        R"""
        Forward.
        Σ (w \in N(v)) A_{v,w} H_w W + b
        """
        assert not torch.isnan(edge_weights).any()
        # compute degree
        A = torch.sparse_coo_tensor(
            edge_indices,         # shape: [2, num_edges]
            edge_weights,         # shape: [num_edges]
            size=(node_embeds.shape[0], node_embeds.shape[0])     # [num_nodes, num_nodes]
        )
        res = torch.sparse.mm(A, node_embeds)
        res = res @ self.weight
        res += self.bias
        res = torch.relu(res)
...
class ModelGCN(Model):
    ...
    def encode(self, /, *ARGS) -> torch.Tensor:
        ...
        for gcn in self.gcns[:-1]:
            #
            # print("node_embeds.shape: {}".format(node_embeds.shape))
            # print("edge_indices.shape: {}".format(edge_indices.shape))
            # print("edge_weights.shape: {}".format(edge_weights.shape))
